...
class MarkDownConcatenator {
    ...
    async getFileDetails() {
        ...
        return Promise.all(fileNames.map(async (fileName) => ({ path: fileName, ...front_matter_1.default(await readFile(fileName, { encoding: "utf8" })) })));
    }
    getFileDetailsSync() {
    ...
    addTitle(file) {
        ...
        if (this.dirNameAsTitle) {
            ...
            dirParts.forEach(part => {
                ...
                if (!this.visitedDirs.has(currentDir)) {
                    ...
                    const dirTitle = lodash_startcase_1.default(part);
                    titleMd += `${dirTitlePrefix} ${dirTitle}${titleSuffix}`;
                    this.visitedDirs.add(currentDir);
                    this.fileTitleIndex.set(path_1.join(this.dir, currentDir), { title: dirTitle, md: titleMd, level });
                }
            });
        }
        const titleFromMeta = this.titleKey && file.attributes && file.attributes[this.titleKey];
        const titleFromFileName = `${lodash_startcase_1.default(path_1.basename(file.path, path_1.extname(file.path)))}`;
        if (titleFromMeta || this.fileNameAsTitle) {
            ...
            fileTitle = gitHubLink(path_1.relative(this.dir, file.path));

 ...
 * @ignore
 */
function splitPaths(pathsCSV) {
    return pathsCSV ? pathsCSV.split(/\s*,\s*/).map(f => path_1.resolve(f)) : [];
}
/** @ignore */
async function exec() {
    const cli = meow_1.default(HELP, { flags: FLAGS, inferType: true });
    const dir = cli.input[0];
    if (!dir || dir.length === 0) {
        console.log(`${HELP}${os_1.EOL}Error: Dir is required`);
        return;
    }
    const flags = {
        ...cli.flags,
    ...
    try {
        ...
        const result = await index_1.default(path, flags);
...
exec();
//# sourceMappingURL=concat-md.js.map